...
def draw_lines(fixed_point, points):
...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        def calculate_angle_to_ar_tag(x_pos, z_pos):
     
            angle_radians = math.atan2(z_pos, x_pos)  # z_pos is used as y, x_pos as x
            angle_degrees = math.degrees(angle_radians)
            
            # If angle is positive, the AR tag is on the left, otherwise it's on the right
            return angle_degrees

    # Main loop inside AR_DRIVE
    while drive_mode == AR_DRIVE:
        ...
        if ar_ID != 99:  # AR태그가 발견되면
            retry_count = 0
            angle_to_ar_tag = calculate_angle_to_ar_tag(x_pos, z_pos)
            
            # 차량의 회전 각도를 설정합니다.
            new_angle = -int(angle_to_ar_tag)
            ...
            print(f"AR ID: {ar_ID}, Z Position: {z_pos}, X Position: {x_pos}")
            print(f"Calculated Angle: {angle_to_ar_tag}, Driving Angle: {new_angle}")
            ...
            retry_count = 0
            if(abs(x_pos)>0.1):
                degree= math.atan(float(z_pos)/x_pos)
                if(x_pos<0):
                     degree= degree +math.pi
                theta =degree*180/math.pi
                new_angle = int(-(theta-90))
            ...
            retry_count += 1
            print(f"Keep going... New Angle: {new_angle}, Retry Count: {retry_count}")
            draw_lines((0, 0), [(0, 0)])
            ...
            # new_angle =int(max(min(new_angle,0.0) ,-50.0))
            # if(debug != 1):        
            #     drive(new_angle, int(new_speed))  
            #     rospy.sleep(0.2)
            # else:
            #     print(new_angle,new_speed)
