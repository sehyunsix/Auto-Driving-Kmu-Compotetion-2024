...
#=============================================
pid = PID(kp=rospy.get_param('pid_kp'), ki=rospy.get_param('pid_ki'), kd=rospy.get_param('pid_kd'),)
...
def check_traffic_sign():
    ...
    if (circles is not None) and (len(circles)==3):
        ...
        for i, (x, y, r) in enumerate(circles):
            ...
            # 이번 원의 밝기가 기존 max원보다 밝으면 이번 원을 max원으로 지정  소팅해서
...
def lane_detect():
    ...
    edge_img = cv2.Canny(blur_gray,40, 50 )
    # edge_img[ :, 290:351] = 0
